...
function configureProductionLogger(options) {
  try {
    const { permittedCSP, cspConfig, useCORSDevOverride = true, useCSP = true, useHelmet = true, aws_key, aws_secret, subscriber, topic_arn, region, useSNSLog = true, useNewrelicLog = true } = options;
    ...
    if (useCORSDevOverride && env === 'development') {
